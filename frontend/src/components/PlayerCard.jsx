...
const PlayerCard = ({ player, isSelected = false, onClick = null }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`h-3 bg-gradient-to-r ${positionColors[playerData.position]}`} />
        ...
        <div className="flex items-start gap-3">
          <div className="relative">
            ...
            <div className="absolute -top-1 -right-1">
              <Badge className={`text-xs px-2 py-1 ${positionBg[playerData.position]} border-0`}>
                {playerData.position}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-1">
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-lg leading-tight">{playerData.name}</h3>
              {playerData.isSubscribed && (
                <CheckCircle className="w-4 h-4 text-green-600" />
              )}
            </div>
            <p className="text-sm text-muted-foreground">{playerData.nationality} • Age {playerData.age}</p>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="flex justify-between">
              <span>PAC</span>
              <span className="font-semibold">{playerData.skills.pace}</span>
            </div>
            <div className="flex justify-between">
              <span>SHO</span>
              <span className="font-semibold">{playerData.skills.shooting}</span>
            </div>
            <div className="flex justify-between">
              <span>PAS</span>
              <span className="font-semibold">{playerData.skills.passing}</span>
            </div>
            <div className="flex justify-between">
              <span>DEF</span>
              <span className="font-semibold">{playerData.skills.defending}</span>
            </div>
            <div className="flex justify-between">
              <span>DRI</span>
              <span className="font-semibold">{playerData.skills.dribbling}</span>
            </div>
            <div className="flex justify-between">
              <span>PHY</span>
              <span className="font-semibold">{playerData.skills.physical}</span>
            </div>
          </div>
        </div>
